                        ...
                        ExecRequest {
                            argv: vec![
                                ...
                                concat!(
                                    ...
                                    "resolver=open('/etc/resolv.conf').read().strip(); ",
                                    "route=open('/proc/net/route').read().strip(); ",
                                    "assert resolver.startswith('nameserver 10.200.'); ",
                                    "dns=resolver.split()[1]; ",
                                    "\ntry: response=urllib.request.urlopen('https://example.com', timeout=10)",
                                    "\nexcept Exception:",
                                    ...
                                    "tcp=socket.create_connection((dns, 53), timeout=2); ",
                                    "tcp.sendall(len(query).to_bytes(2, 'big') + query); ",
                                    "reader=tcp.makefile('rb'); ",
                                    "answer_length=int.from_bytes(reader.read(2), 'big'); ",
                                    "answer=reader.read(answer_length); tcp.close(); ",
                                    "assert len(answer) == answer_length and answer[:2] == query[:2] and answer[2] & 128; ",
